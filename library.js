(function(module) {
    ...
    Widget.renderWidget = function(widget, callback) {
        Widget.settings(function(err, settings) {
            var html = Widget.templates['gameservers.tpl'];
    ...
    Widget.settings = function(settings, callback) {
        ...
        if (settings != null) {
            ...
            meta.settings.set('gameservers', settings, function() {
                Widget.settings(callback);
            });
        } else {
            meta.settings.get('gameservers', function(err, settings) {
	...
	var done = function() {
	        ...
	        var html = templates.parse(Widget.templates['admin/gameservers.tpl'], data);
	
	        widgets.push({
	            widget: "gameservers",
	            name: "gameservers",
	            description: "NodeBB Gameservers widget",
	            content: html
	        });
    ...
    var addserver = function(data, callback) {
        ...
        Widget.settings(Widget._settings, function() {
            Widget.emit('gameservers.serveradded', server);
    ...
    var rmserver = function(data, callback) {
        ...
        Widget.settings(Widget._settings, function() {
            Widget.emit('gameservers.serverrmed', data);
            callback(null, data);
        });
    };
    ...
    var fetchserver = function(key, callback) {
        ...
        if (server) {
            gamedig.query({
                ...
                state.key = key;
                Widget.emit('gameservers.serverfetched', state);
                callback(null, state);
            });
        } else {
    ...
    var fetchallservers = function(callback) {
        ...
        async.eachLimit(Object.keys(Widget._settings.servers), 3, function(key, next) {
			...
			Widget.emit('gameservers.serversfetchedall', servers);
    ...
    Widget.init = function(app, middleware, controllers, callback) {
        var templatesToLoad = ['gameservers.tpl', 'admin/gameservers.tpl'];
        ...
        Widget.settings(function() {
            ...
            var prefix = '/api/admin/widgets/gameservers';
